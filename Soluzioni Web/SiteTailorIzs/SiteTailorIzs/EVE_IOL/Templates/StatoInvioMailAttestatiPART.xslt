...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:asp="remove" xmlns:ajaxToolkit="remove">
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="mytable">
          <tr>
            ...
            <td class="mydata">
              <div class="mylist">
                <xsl:choose>
                  <xsl:when test="count(persone/persona[@fl_inviomailattpartok='1'])&gt;0">
                    <xsl:apply-templates select="persone/persona[@fl_inviomailattpartok='1']" mode="done" />
                  </xsl:when>
                  <xsl:otherwise>
                    <div>[nessuno]</div>
                  </xsl:otherwise>
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="mytable">
          <tr>
            ...
            <td class="mydata">
              <div class="mylist">
                <xsl:choose>
                  <xsl:when test="count(persone/persona[@fl_inviomailattpartok='0'])&gt;0">
                    <xsl:apply-templates select="persone/persona[@fl_inviomailattpartok='0']" mode="done" />
                  </xsl:when>
                  <xsl:otherwise>
                    <div>[nessuno]</div>
                  </xsl:otherwise>
              ...
              <div style="padding-top:3px;">
                <asp:Button runat="server" ID="lnkInvioUlteriore" Text="Effettua invio ulteriore per questi nominativi" CssClass="btnlink">
                  <xsl:attribute name="enabled">
                    <xsl:choose>
                      <xsl:when test="count(persone/persona[@fl_inviomailattpartok='0'])">
                ...
                <xsl:if test="count(persone/persona[@fl_inviomailattpartok='0'])">
                  <div style="padding-top:3px;">
  ...
  <xsl:template match="persona" mode="done">
    <div>
      ...
      <xsl:choose>
        <xsl:when test="@fl_inviomailattpartok='1'">
          <!-- mail inviata -->
          <span class="mymail">
            <xsl:value-of select="' - invio: '"/>
            <xsl:call-template name="dataoraDDMMYYHHMM">
              <xsl:with-param name="dataora" select="@dt_inviomailattpart" />
            </xsl:call-template>
          </span>
        </xsl:when>
        <xsl:otherwise>
          <!-- mail non inviata -->
          <xsl:choose>
            <xsl:when test="@dt_inviomailattpart!=''">
            ...
            <xsl:otherwise>
              ...
              <xsl:value-of select="' - '"/>
              <span style="color:#ff6600">Presenza registrata o categoria attivata dopo ultimo invio</span>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:otherwise>
      </xsl:choose>
